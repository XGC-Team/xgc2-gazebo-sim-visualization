...
if [[ -n "${XGC2_APT_OVERLAY_URL:-}" ]]; then
  ...
  git -C "${source_dir}" remote add origin https://github.com/lxk36/xgc2-robot-visualization.git
  git -C "${source_dir}" fetch --depth 1 origin "${XGC2_ROBOT_VISUALIZATION_STANDALONE_SOURCE_REF}"
  git -C "${source_dir}" checkout -q --detach FETCH_HEAD

  source_version="$(sed -n 's/^version:[[:space:]]*//p' "${source_dir}/.xgc2/product.yml" | head -n 1)"
  ...
  rsync -a --delete "${source_dir}/" "${build_workspace}/src/xgc2_robot_visualization/"
  (
    cd "${build_workspace}"
  ...
  PACKAGE_VERSION="${source_version}" \
    "${source_dir}/.xgc2/scripts/package_debs.sh" \
      ...
      --output-dir "${deb_dir}"
  apt-get install -y --no-install-recommends "${deb_dir}/${package_name}_"*.deb
